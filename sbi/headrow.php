    ...
    if ($nrows > 0) {
      while ($get_column=$res->fetch_assoc()) {
        //  echo '<header onclick="javascript:location.href=\'ebal.php \'">';
         ...
         echo '</div>';
         echo '<div class="w3-col w3-black w3-container" style="width:20%"><p><center>'. $get_column['cash'].'</center></p></div>';
         ...
         echo '<div class="w3-col w3-black w3-container" style="width:20%"><p>'. $get_column['account'].'</p></div>';
         echo '</header>';
       }
     }
    mysqli_close($con);
